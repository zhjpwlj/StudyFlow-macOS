        ...
        <div className="flex items-center gap-2 p-1 rounded-lg bg-gray-100 dark:bg-slate-700/50">
          {['all', 'active', 'completed'].map((f) => (
            <button key={f} onClick={() => setFilter(f as any)}
              className={`px-3 py-1 text-sm font-medium capitalize rounded-md transition-colors ${filter === f ? 'bg-white dark:bg-slate-600 text-[var(--accent-color)] dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:bg-white/50'}`}>
              {f}
            </button>
          ))}
        </div>
            ...
            <div className="space-y-1">
              {filteredTasks.map(task => (
                <div key={task.id} className="group flex items-center gap-3 p-2 hover:bg-gray-100 dark:hover:bg-slate-700/50 rounded-lg transition-colors">
                  <button onClick={() => onToggleTask(task.id)} className={`flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center ${task.completed ? 'bg-emerald-500 border-emerald-500' : 'border-gray-300 dark:border-gray-500 hover:border-[var(--accent-color)]'}`}>
        ...
        <div className="p-2 border-t border-gray-200 dark:border-slate-700/50 bg-gray-50 dark:bg-slate-900/50">
          <form onSubmit={handleSubmit} className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
              ...
              <button type="submit" disabled={!newTaskTitle.trim()} className="px-3 py-1 bg-[var(--accent-color)] text-white rounded-lg text-sm font-medium hover:bg-[var(--accent-color-hover)] disabled:opacity-50">Add</button>
            </div>
